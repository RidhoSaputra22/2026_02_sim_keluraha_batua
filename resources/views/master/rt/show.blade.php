<x-layouts.app :title="'RT ' . str_pad($rt->nomor, 3, '0', STR_PAD_LEFT) . ' / RW ' . str_pad($rt->rw->nomor, 3, '0', STR_PAD_LEFT)">
    <x-slot:header>
        <x-layouts.page-header title="RT {{ str_pad($rt->nomor, 3, '0', STR_PAD_LEFT) }}" description="RW {{ str_pad($rt->rw->nomor, 3, '0', STR_PAD_LEFT) }} — Kel. {{ $rt->rw->kelurahan->nama ?? '-' }}">
            <x-slot:actions>
                <x-ui.button type="ghost" size="sm" href="{{ route('master.rt.index') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                    Kembali
                </x-ui.button>
                <x-ui.button type="primary" size="sm" href="{{ route('master.rt.edit', $rt) }}">
                ...
                <x-ui.button type="error" size="sm" :outline="true" @click="$dispatch('confirm-delete', { action: '{{ route('master.rt.destroy', $rt) }}', message: 'Hapus RT {{ str_pad($rt->nomor, 3, '0', STR_PAD_LEFT) }}? Pastikan tidak ada data penduduk terkait.' })">
                    Hapus
                </x-ui.button>
            </x-slot:actions>
            ...
            <x-ui.card title="Info Ringkas">
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <a href="{{ route('master.rw.show', $rt->rw) }}" class="font-semibold link link-primary">
