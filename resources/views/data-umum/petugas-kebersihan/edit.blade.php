<x-layouts.app :title="'Edit Petugas Kebersihan'">
    <x-slot:header>
        <x-layouts.page-header title="Edit Petugas Kebersihan" description="Perbarui data petugas kebersihan">
            <x-slot:actions>
    ...
    <x-ui.card>
        <form method="POST" action="{{ route('data-umum.petugas-kebersihan.update', $petugasKebersihan) }}">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <x-ui.select label="Status" name="status" placeholder="Pilih Status" :options="['aktif' => 'Aktif', 'tidak_aktif' => 'Tidak Aktif']" selected="{{ old('status', $petugasKebersihan->status) }}" />
            </div>
