<x-layouts.app :title="'Edit Data PBB'">
    <x-slot:header>
        <x-layouts.page-header title="Edit Data PBB" description="Perbarui data Pajak Bumi dan Bangunan">
            <x-slot:actions>
    ...
    <x-ui.card>
        <form method="POST" action="{{ route('data-umum.pbb.update', $pbb) }}">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <x-ui.select label="Kelurahan" name="kelurahan_id" placeholder="Pilih Kelurahan"
                    :options="$kelurahanOptions" selected="{{ old('kelurahan_id', $pbb->kelurahan_id) }}" required />
                <x-ui.input label="Nama Wajib Pajak" name="nama_wajib_pajak" placeholder="Nama lengkap wajib pajak"
                    value="{{ old('nama_wajib_pajak', $pbb->nama_wajib_pajak) }}" required />
            ...
            <div class="mb-6">
                <x-ui.textarea name="keterangan" placeholder="Keterangan tambahan (opsional)"
                    value="{{ old('keterangan', $pbb->keterangan) }}" />
            </div>
