<x-layouts.app :title="'Edit Data RT/RW'">
    <x-slot:header>
        <x-layouts.page-header title="Edit Data RT/RW" description="Ubah data pengurus {{ $wilayah->jabatan->nama ?? '' }} - {{ $wilayah->penduduk->nama ?? '' }}">
            <x-slot:actions>
    ...
    <x-ui.card>
        ...
        <form method="POST" action="{{ route('master.wilayah.update', $wilayah) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <x-ui.select label="Penduduk" name="penduduk_id" required :options="$pendudukList->mapWithKeys(fn($p) => [$p->id => $p->nik . ' - ' . $p->nama])->toArray()" selected="{{ old('penduduk_id', $wilayah->penduduk_id) }}" />
                <x-ui.select label="Kelurahan" name="kelurahan_id" required :options="$kelurahanList->mapWithKeys(fn($k) => [$k->id => $k->nama])->toArray()" selected="{{ old('kelurahan_id', $wilayah->kelurahan_id) }}" />
            </div>

            {{-- Data Jabatan --}}
            <h3 class="text-lg font-semibold mb-4 border-b pb-2">Data Jabatan</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                <x-ui.select label="Jabatan" name="jabatan_id" required :options="$jabatanList->mapWithKeys(fn($j) => [$j->id => $j->nama])->toArray()" selected="{{ old('jabatan_id', $wilayah->jabatan_id) }}" />
                <x-ui.select label="RW" name="rw_id" :options="$rwList->mapWithKeys(fn($r) => [$r->id => 'RW ' . $r->nomor])->toArray()" selected="{{ old('rw_id', $wilayah->rw_id) }}" />
                <x-ui.select label="RT" name="rt_id" :options="$rtList->mapWithKeys(fn($r) => [$r->id => 'RT ' . $r->nomor . ' / RW ' . ($r->rw->nomor ?? '-')])->toArray()" selected="{{ old('rt_id', $wilayah->rt_id) }}" />
                <x-ui.input label="Tanggal Mulai" name="tgl_mulai" type="date" value="{{ old('tgl_mulai', $wilayah->tgl_mulai) }}" />
                <x-ui.select label="Status" name="status" required :options="\App\Enums\StatusAktifEnum::options()" selected="{{ old('status', strtolower((string) ($wilayah->status ?? \App\Enums\StatusAktifEnum::AKTIF->value))) }}" />
            </div>
