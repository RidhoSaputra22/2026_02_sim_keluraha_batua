<x-layouts.app :title="'Data Wilayah RT/RW'">
    <x-slot:header>
        <x-layouts.page-header title="Data Wilayah RT/RW" description="Kelola data RT dan RW">
            <x-slot:actions>
                <x-ui.button type="primary" size="sm" href="{{ route('admin.wilayah.create') }}">
    ...
    <x-ui.card class="mb-6">
        <form method="GET" action="{{ route('admin.wilayah.index') }}" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                <x-ui.button type="ghost" size="md" href="{{ route('admin.wilayah.index') }}">Reset</x-ui.button>
            </div>
        </form>
    </x-ui.card>
                    ...
                    <tr class="hover">
                        ...
                        <td>
                            <div class="flex justify-end gap-1">
                                <x-ui.button type="ghost" size="xs" href="{{ route('admin.wilayah.edit', $w) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </x-ui.button>
                                <form method="POST" action="{{ route('admin.wilayah.destroy', $w) }}" onsubmit="return confirm('Hapus data wilayah ini?')">
                                    @csrf @method('DELETE')
