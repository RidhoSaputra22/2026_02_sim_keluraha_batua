{{-- SECTION 4: DEMOGRAFI (Gender, Agama, Gol Darah, Kawin) --}}
<div class="mb-2">
    ...
    <p class="text-sm text-base-content/50 mb-4">Proporsi penduduk berdasarkan karakteristik</p>
</div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">

    {{-- Chart: Jenis Kelamin --}}
    <x-ui.card title="Jenis Kelamin" compact>
        <div class="h-52 flex items-center justify-center">
            <canvas id="chartGender"></canvas>
        </div>
        <div class="flex justify-center gap-4 mt-2 text-xs text-base-content/60">
            @foreach($genderData as $label => $val)
    ...
    <x-ui.card title="Golongan Darah" compact>
        <div class="h-52 flex items-center justify-center">
            <canvas id="chartGolDarah"></canvas>
        </div>
        <div class="flex flex-wrap justify-center gap-x-3 gap-y-1 mt-2 text-xs text-base-content/60">
            @foreach($golDarahData as $label => $val)
            <span>{{ strtoupper($label) }}: <strong>{{ $val }}</strong></span>
            @endforeach
        </div>
    </x-ui.card>
