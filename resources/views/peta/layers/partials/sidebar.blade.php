...
<div
    class="layer-sidebar w-full lg:w-80 xl:w-96 bg-base-100 border-t lg:border-t-0 lg:border-l border-base-200 flex flex-col">
        ...
        <div class="px-1 py-2">
            <template x-for="(layer, index) in layers" :key="layer.id">
                <div class="tree-layer-group mb-0.5 mx-1">
                    {{-- Layer parent node --}}
                    <div class="layer-item rounded-lg px-3 py-2.5 cursor-pointer group hover:shadow" :class="{
                        ...
                        @drop="onDrop($event, index)" @dragend="onDragEnd($event)" @click="toggleTreeExpand(layer)">

                        <div class="flex items-center gap-2">
                            {{-- Tree expand/collapse toggle --}}
                            <button class="flex-shrink-0 tree-toggle-btn" @click.stop="toggleTreeExpand(layer)"
                                :title="layer._expanded ? 'Ciutkan' : 'Perluas'">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-3.5 w-3.5 transition-transform duration-150"
                                    :class="{ 'rotate-90': layer._expanded }" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 5l7 7-7 7" />
                                </svg>
                            ...
                            {{-- Visibility toggle --}}
                            <button class="flex-shrink-0" @click.stop="toggleLayerVisibility(layer)"
                                :title="layer.visible ? 'Sembunyikan' : 'Tampilkan'">
                                <svg x-show="layer.visible" xmlns="http://www.w3.org/2000/svg"
                                    class="h-4 w-4 text-base-content/70" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <svg x-show="!layer.visible" xmlns="http://www.w3.org/2000/svg"
                                    class="h-4 w-4 text-base-content/30" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span class="badge badge-ghost"
                                        style="font-size: 9px; padding: 0 4px; height: 14px;"
                                        x-text="layer.polygons_count + 'p'"></span>
                                </div>
                            </div>
                            ...
                            <div class="dropdown dropdown-end flex-shrink-0" @click.stop>
                                ...
                                <ul tabindex="0"
                                    class="dropdown-content menu menu-xs bg-base-100 rounded-lg shadow-xl border border-base-200 w-44 z-50">
                    ...
                    <div x-show="layer._expanded" x-transition:enter="transition ease-out duration-150"
                        x-transition:enter-start="opacity-0 -translate-y-1"
                        x-transition:enter-end="opacity-100 translate-y-0"
                        x-transition:leave="transition ease-in duration-100"
                        x-transition:leave-start="opacity-100 translate-y-0"
                        x-transition:leave-end="opacity-0 -translate-y-1" class="tree-children">
                        <template x-for="(poly, idx) in getLayerPolygons(layer)" :key="poly.id || ('new-' + idx)">
                            <div class="polygon-tree-item flex items-center gap-2 pl-9 pr-3 py-1.5 cursor-pointer rounded-md"
                                :data-poly-id="poly.id" :class="{
                                    'polygon-editing': editingPolygon?.id === poly.id && editingPolygon?.layerId === layer.id,
                                    'polygon-highlighted': highlightedPolygonId != null && Number(highlightedPolygonId) === Number(poly.id),
                                    'polygon-drag-over': _polyDragOver?.layerId === layer.id && _polyDragOver?.index === idx
                                }" draggable="true" @dragstart.stop="onPolyDragStart($event, layer, idx)"
                                @dragover.prevent.stop="onPolyDragOver($event, layer, idx)"
                                @dragleave.stop="onPolyDragLeave($event)" @drop.stop="onPolyDrop($event, layer, idx)"
                                @dragend.stop="onPolyDragEnd($event)"
                                @click.stop="selectPolygonForEdit(layer, poly, idx)">

                                {{-- Drag handle for polygon --}}
                                <div class="drag-handle flex-shrink-0" title="Seret untuk mengubah urutan" @click.stop>
                                ...
                                <span class="text-xs flex-1 min-w-0 truncate"
                                    x-text="poly.nama || 'Polygon ' + (idx + 1)"></span>

                                {{-- Polygon context menu (3-dot) --}}
                                <div class="dropdown dropdown-end flex-shrink-0" @click.stop>
                                    <button tabindex="0"
                                        class="btn btn-ghost btn-xs btn-circle opacity-50 hover:opacity-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none"
                                    ...
                                    <ul tabindex="0"
                                        class="dropdown-content menu menu-xs bg-base-100 rounded-lg shadow-xl border border-base-200 w-40 z-50">
                                        <li><a @click="openEditPolygonModal(layer, poly)">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <li><a @click="zoomToPolygon(layer, poly)">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                        ...
                                        <div class="divider my-0"></div>
                                        <li><a class="text-error" @click="deletePolygon(poly, idx)">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
            ...
            <template x-if="layers.length === 0">
                <div class="text-center py-8 px-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 mx-auto text-base-content/20 mb-3"
        ...
        <div class="px-3 py-2 border-t border-base-200 flex-shrink-0 bg-primary/5">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-primary animate-pulse" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
