...
{{-- SECTION 2: PETA WILAYAH + MUTASI + AKSI CEPAT --}}
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">

    {{-- Peta Wilayah --}}
    <div class="lg:col-span-2">
        <x-ui.card title="Peta Wilayah {{ $wilayahLabel }}" compact>
            <div class="relative">
    ...
    {{-- Mutasi + Aksi Cepat --}}
    <div class="space-y-6">
        <x-ui.card title="Mutasi Warga" compact>
            ...
            <div class="space-y-2">
                ...
                @foreach ($mutasiItems as $item)
                    <div class="flex items-center justify-between rounded-lg px-3 py-2 {{ $item['bg'] }}">
                        <div class="flex items-center gap-2">
                            <svg class="h-4 w-4 {{ $item['color'] }}" fill="none" viewBox="0 0 24 24"
                ...
                <div class="pt-2 border-t border-base-300">
                    <div class="flex items-center justify-between text-xs text-base-content/50 px-1">
                        <span>Total</span>
                        <span
                            class="font-bold text-base-content text-base">{{ $mutasiLahir + $mutasiMeninggal + $mutasiDatang + $mutasiPindah }}</span>
                    </div>
                </div>
            </div>
        </x-ui.card>
        ...
        <x-ui.card title="Aksi Cepat" compact>
            <div class="space-y-2">
                ...
                <x-ui.button type="accent" size="sm" class="w-full justify-start gap-2"
                    href="{{ route('usaha.create') }}" :outline="true">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <x-ui.button type="ghost" size="sm" class="w-full justify-start gap-2"
                    href="{{ route('rtrw.laporan.index') }}" :outline="true">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
