<x-layouts.app :title="'Data Penduduk'">
    ...
    <x-ui.card class="mb-6">
        <form method="GET" action="{{ route('kependudukan.penduduk.index') }}" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <x-ui.input name="search" placeholder="Cari NIK, nama, alamat..." value="{{ request('search') }}" />
            </div>
            <div class="w-full md:w-32">
                <x-ui.select name="rw" placeholder="Semua RW"
                    :options="$rwList->mapWithKeys(fn($r) => [$r->id => 'RW ' . $r->nomor])->toArray()"
                    selected="{{ request('rw') }}" />
            </div>
            <div class="w-full md:w-32">
                <x-ui.select name="rt" placeholder="Semua RT"
                    :options="$rtList->mapWithKeys(fn($r) => [$r->id => 'RT ' . $r->nomor . '/RW ' . ($r->rw->nomor ?? '-')])->toArray()"
                    selected="{{ request('rt') }}" />
            </div>
            <div class="w-full md:w-32">
                <x-ui.select name="jenis_kelamin" placeholder="Semua JK"
                    :options="['L' => 'Laki-laki', 'P' => 'Perempuan']" selected="{{ request('jenis_kelamin') }}" />
            </div>
            <div class="flex gap-2">
                <x-ui.button type="primary" size="md" :isSubmit="true">Cari</x-ui.button>
                <x-ui.button type="ghost" size="md" href="{{ route('kependudukan.penduduk.index') }}">Reset
                </x-ui.button>
            </div>
        </form>
    </x-ui.card>
                        ...
                        <td>
                            <div class="flex justify-end gap-1">
                                <x-ui.button type="info" size="xs" :outline="true"
                                    href="{{ route('kependudukan.penduduk.show', $p) }}">
                                ...
                                <x-ui.button type="ghost" size="xs"
                                    href="{{ route('kependudukan.penduduk.edit', $p) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <x-ui.button type="error" size="xs" :outline="true"
                                    @click="$dispatch('confirm-delete', { action: '{{ route('kependudukan.penduduk.destroy', $p) }}', message: 'Hapus data penduduk ini?' })">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
